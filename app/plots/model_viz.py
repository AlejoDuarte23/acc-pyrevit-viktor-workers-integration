...
def add_beam_mesh(fig: go.Figure, verts: np.ndarray, color: str) -> None:
    ...
    for a, b, c, d in quads:
        # forward faces (two tris)
        i_list.extend([a, a])
        j_list.extend([b, c])
        k_list.extend([c, d])
        # back faces (two tris reversed)
        i_list.extend([a, a])
        j_list.extend([c, d])
        k_list.extend([b, c])
...
def plot_3d_model(
    ...
    # --- colour grouping by SECTION NAME (so identical names share one colour) -----
    cs_ids = sorted({m["cross_section_id"] for m in members.values()})
    # Extract names (fallback to id)
    name_by_cs_id = {
        cs_id: (cross_sections[cs_id].get("name") or cross_sections[cs_id].get("Description") or f"Section {cs_id}")
        for cs_id in cs_ids
    }
    ordered_unique_names: list[str] = []
    for cs_id in cs_ids:
        nm = name_by_cs_id[cs_id]
        if nm not in ordered_unique_names:
            ordered_unique_names.append(nm)
    color_map_by_name = {
        nm: PASTEL_PALETTE[i % len(PASTEL_PALETTE)] for i, nm in enumerate(ordered_unique_names)
    }
    # Legend labels -> unique names only
    legend_names = ordered_unique_names
    ...
    for member in members.values():
        ...
        width, height = float(cs.get("h", 0.3)), float(cs.get("h", 0.3))
    ...
    for nm in legend_names:
        fig.add_trace(
            go.Scatter3d(
                ...
                marker=dict(symbol="square", size=10, color=color_map_by_name[nm]),
                name=nm, hoverinfo="none", showlegend=True
            )
        )
